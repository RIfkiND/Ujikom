<!-- filepath: resources/views/Dashboard/tarif/edit.blade.php -->
<x-app-layout>
    <x-slot name="header">
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 bg-white p-6 shadow-md rounded-md">
            <h1 class="text-2xl font-bold mb-6 text-black">Edit Tarif</h1>
            ...
            <form action="{{ route('tarif.update', $tarif->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="jenis_plg" id="jenis_plg"
                        ...
                        value="{{ old('jenis_plg', $tarif->jenis_plg) }}" required>
                    @error('jenis_plg')
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="biaya_beban" id="biaya_beban"
                        ...
                        value="{{ old('biaya_beban', $tarif->biaya_beban) }}" required>
                    @error('biaya_beban')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="tarif_kwh" id="tarif_kwh"
                        ...
                        value="{{ old('tarif_kwh', $tarif->tarif_kwh) }}" required>
                    @error('tarif_kwh')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
...
<!-- filepath: d:\ujikom\ujikom_rifki\resources\views\Dashboard\tarifs\edit.blade.php -->
{{-- <x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-black leading-tight">
            {{ __('Edit Tarif') }}
                ...
                <div class="mb-4">
                    ...
                    <select name="jenis_plg" id="jenis_plg" class="form-select w-full border-gray-300 text-black @error('jenis_plg') border-red-500 @enderror" required>
                        @foreach ($jenis as $item)
                            <option value="{{ $item->id }}" {{ old('jenis_plg', $tarif->jenis_plg) == $item->id ? 'selected' : '' }}>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>
