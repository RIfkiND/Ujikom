...
<body class="bg-gray-50 text-gray-800 flex flex-col min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center">
                    <a href="{{ url('/') }}" class="text-2xl font-bold">Ujikom</a>
                </div>
    ...
    <!-- Main Content -->
    <main class="flex-grow w-full max-w-4xl mx-auto bg-white p-8 shadow-lg rounded-lg mt-8">

        <!-- Header -->
        <header class="text-center mb-8">
            <h1 class="text-3xl font-bold text-blue-600 tracking-wide mb-2">History Listrik</h1>
            <p class="text-sm text-gray-500">Cek Pemakaian Pelanggan Berdasarkan No Kontrol</p>
        </header>

        <!-- Search Form -->
        <form action="{{ url('/') }}" method="GET" class="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        @if (request()->has('search'))
            @if (isset($pelanggan) && $pelanggan)
                <!-- Pelanggan Info -->
                <div class="mb-8">
                    <h2 class="text-xl font-semibold mb-3">🔎 Hasil Pencarian</h2>
                    <div class="bg-blue-50 border border-blue-100 rounded-lg p-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p><strong>Jenis:</strong> {{ $pelanggan->tarif->jenis_plg }}</p>
                            </div>
                            <!-- Right Column -->
                            <div>
                ...
                <!-- Filters -->
                <form action="{{ url('/') }}" method="GET" class="flex flex-wrap gap-4 mb-6">
                ...
                <!-- Pemakaian Table -->
                <h3 class="text-lg font-semibold mb-4 mt-6">📊 Riwayat Pemakaian</h3>
                        ...
                        <table
                            ...
                            <thead class="bg-gray-100 text-gray-600 uppercase text-xs">
                                <tr>
                                    ...
                                    <th class="px-4 py-3">Pakai (kWh)</th>
                            ...
                            <tbody>
                                @foreach ($pemakaians as $pemakaian)
                                    <tr class="border-t hover:bg-gray-50">
                                        <td class="px-4 py-3">{{ $pemakaian->tahun }}</td>
                                        <td class="px-4 py-3">
                                            {{ \Carbon\Carbon::create()->month($pemakaian->bulan)->translatedFormat('F') }}
                                        </td>
                                        <td class="px-4 py-3">{{ $pemakaian->meter_awal }}</td>
                                        <td class="px-4 py-3">{{ $pemakaian->meter_akhir }}</td>
                                        <td class="px-4 py-3">{{ $pemakaian->jumlah_pakai }}</td>
                                        <td class="px-4 py-3">Rp {{ number_format($pemakaian->biaya_beban_pemakaian) }}
                                        </td>
                                        <td class="px-4 py-3">Rp {{ number_format($pemakaian->biaya_pemakaian) }}</td>
                                        <td class="px-4 py-3">Rp {{ number_format($pemakaian->total_bayar) }}</td>
                ...
                @else
                    <div class="mt-4 text-gray-500 italic">Tidak ada data pemakaian ditemukan.</div>
                @endif
            @else
                <div class="bg-red-50 border border-red-200 text-red-600 rounded p-4">
                    ⚠️ Pelanggan dengan nomor kontrol tersebut tidak ditemukan.
                </div>
            @endif
        @endif
    ...
    <footer class="bg-gray-800 text-white py-4 mt-8">
        <div class="max-w-7xl mx-auto text-center">
            <p>&copy; {{ date('Y') }} Ujikom. All rights reserved. Developed by Rifki N D.</p>
        </div>
    </footer>
